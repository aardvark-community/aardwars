    ...
    let treeWorld (win : IRenderWindow) (tex : ITexture) (tree : Octree<BoxInfo>) (playerHeight : float) =
            ...
            if iter > 10 then
                ...
                if Fun.IsTiny len then
                    ...
                    if hits.Length > 0 then
                        ...
                        //let (minDist,_) = hits.[0]
                        //let closest = hits |> Array.takeWhile (fun (t,_) -> Fun.ApproximateEquals(minDist, t, 1E-8))
                    
                        //let planes = 
                        //    closest |> Array.collect (fun (t,b) ->
                        //        let lb = b.BoundingBox
                        //        let pt = ray.GetPointOnRay t

                        //        let eps = 1E-8

                        //        [|  
                        //            if Fun.ApproximateEquals(pt.X, lb.Max.X, 1E-10) && d.X < 0.0 then Plane3d(V3d.IOO, lb.Max.X)
                        //            elif Fun.ApproximateEquals(pt.X, lb.Min.X, 1E-10) && d.X > 0.0 then Plane3d(-V3d.IOO, -lb.Min.X)
                   
                        //            if Fun.ApproximateEquals(pt.Y, lb.Max.Y, 1E-10) && d.Y < 0.0 then Plane3d(V3d.OIO, lb.Max.Y)
                        //            elif Fun.ApproximateEquals(pt.Y, lb.Min.Y, 1E-10) && d.Y > 0.0 then Plane3d(-V3d.OIO, -lb.Min.Y)
                  
                        //            if Fun.ApproximateEquals(pt.Z, lb.Max.Z, 1E-10) && d.Z < 0.0 then Plane3d(V3d.OOI, lb.Max.Z)
                        //            elif Fun.ApproximateEquals(pt.Z, lb.Min.Z, 1E-10) && d.Z > 0.0 then Plane3d(-V3d.OOI, -lb.Min.Z)
                        //        |]
                        //    )
                        //    |> Array.distinct
                ...
                //if hits.Length > 0 then
                //    let (minDist,_) = hits.[0]
                //    //let closest = hits |> Array.takeWhile (fun (t,_) -> Fun.ApproximateEquals(minDist, t, 1E-6))

                //    let planes = 
                //        hits |> Array.collect (fun (t,b) ->
                //            let b = b.BoundingBox
                //            let lb = Box3d(b.Min - V3d(Octree.eps,Octree.eps,Octree.eps), b.Max + V3d(Octree.eps, Octree.eps, Octree.eps))
                //            let pt = ray.GetPointOnRay t
                ...
                //            // (b.Min.X - 1)*-1 - -b.Min.X =1

                //            let pp = 
                //                [|  
                //                    if Fun.ApproximateEquals(pt.X, lb.Max.X, eps) && d.X < 0.0 then Plane3d(V3d.IOO, lb.Max.X)
                //                    elif Fun.ApproximateEquals(pt.X, lb.Min.X, eps) && d.X > 0.0 then Plane3d(-V3d.IOO, -lb.Min.X)
                   
                //                    if Fun.ApproximateEquals(pt.Y, lb.Max.Y, eps) && d.Y < 0.0 then Plane3d(V3d.OIO, lb.Max.Y)
                //                    elif Fun.ApproximateEquals(pt.Y, lb.Min.Y, eps) && d.Y > 0.0 then Plane3d(-V3d.OIO, -lb.Min.Y)
                  
                //                    if Fun.ApproximateEquals(pt.Z, lb.Max.Z, eps) && d.Z < 0.0 then Plane3d(V3d.OOI, lb.Max.Z)
                //                    elif Fun.ApproximateEquals(pt.Z, lb.Min.Z, eps) && d.Z > 0.0 then Plane3d(-V3d.OOI, -lb.Min.Z)
                //                |]
                //            pp
                //            //if pp.Length > 0 then   
                //            //    pp
                ...
                //    | 1 ->
                //        Log.line "1 face: %0A" planes
                //        let p1 = p1.GetClosestPointOn(planes.[0])
                //        ret p1
                //    | 2 ->
                //        Log.line "2 faces: %0A" planes
                //        let mutable r = Unchecked.defaultof<_>
                //        if planes.[0].Intersects(planes.[1], &r) then
                //            let p1 = p1.GetClosestPointOn(r)
                //            ret p1
                //        else
                //            Log.error "parallel planes"
                //            ret p1
                //    | 3 ->
                //        Log.line "3 faces: %0A" planes
                //        let mutable pt = V3d.Zero
                //        if planes.[0].Intersects(planes.[1], planes.[2], &pt) then
                //            ret pt
                //        else
                //            Log.error "no intersection point"
                //            ret p1
        ...
        let hit (p0 : V3d) (p1 : V3d) =
            if Fun.ApproximateEquals(p0, p1) then
                ...
                //let mutable p1 = p1
                //for h in [playerHeight; 0.5; 0.0] do
                //    let p0 = p0 - V3d(0.0, 0.0, h)
                //    let r = p1 - V3d(0.0, 0.0, h)
                //    let r = realHit 0 p0 r
